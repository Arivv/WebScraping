...
setwd("C:/Users/Lan/Desktop/RRRRRRRRRRRRRRRRRRRRRRRRRRR")
AllHomeLinks <-dget("AllHomeLInks.R")

#Num <- length(AllHomeLinks)
StartNum <- 1
EndNum <- length(AllHomeLinks)
ResultFile <-  as.data.frame(matrix(ncol=22, nrow=EndNum-StartNum +1))
...
for (n in 1:(EndNum-StartNum +1)){
  ...
  Price_Each_SQFT <- as.numeric(Price_Each_SQFT)
  ...
  Floor_Size <- as.numeric(Floor_Size)
  ...
  Built_Year <- as.numeric(Built_Year)
  ...
  Last_Remodel_Year <- as.numeric(Last_Remodel_Year)
...
##Add Column Names and Save RsultFile into a Txt File
...
write.table(ResultFile, "Indianapolis.txt", sep="\t")
